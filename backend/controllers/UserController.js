...
module.exports = class UserController {
  ...
  static async register(req, res) {
    ...
    if(password !== confirmpassword) {
      res.status(422).json({message: 'The passwords entered do not match'})
      return
    }
  ...
  static async login(req, res) {
    ...
    // check if send password match with db password
  ...
  static async checkUser(req, res) {
    ...
    if(req.headers.authorization) {
      ...
      const decoded = jwt.verify(token, 'ENV_tokenSecretKey')
  ...
  static async updateUser(req, res) {
    ...
    if(user.email !== email && userExists) {
      res.status(422).json({message: 'There is already another user using this email'})
      return
    }
